...
async function ensureSmartBudgetColumns() {
  await addColumnIfMissing("budgets", "travel_mode", "VARCHAR(80) NULL");
  await addColumnIfMissing("budgets", "travel_fare", "DECIMAL(12,2) DEFAULT 0");
  await addColumnIfMissing("budgets", "emergency_percent", "DECIMAL(5,2) DEFAULT 12");
  await addColumnIfMissing("budgets", "economy_total", "DECIMAL(12,2) DEFAULT 0");
  await addColumnIfMissing("budgets", "comfort_total", "DECIMAL(12,2) DEFAULT 0");
  await addColumnIfMissing("budgets", "premium_total", "DECIMAL(12,2) DEFAULT 0");
  await addColumnIfMissing("budgets", "recommended_plan", "VARCHAR(80) NULL");
}
...
async function initDatabase() {
  ...
  await ensureSmartBudgetColumns();
...
app.post("/api/budget", requireUserOrGuest, async (req, res) => {
  try {
    ...
    const numTravelFare = Number(travelFare) || 0;
    ...
    const bufferPercent = Number(emergencyPercent) || 12;

    const fareTotal = numTravelFare * numTravellers;
    const hotelTotal = numHotelCost * numNights;
    const foodTotal = numFoodCost * (numNights + 1) * numTravellers;
    const subtotal = fareTotal + hotelTotal + foodTotal + numTransportCost + numTicketCost + numShoppingCost;
    const emergencyBuffer = subtotal * (bufferPercent / 100);
    const grandTotal = subtotal + emergencyBuffer;
    const perPerson = grandTotal / numTravellers;

    // Plan multipliers create a quick comparison for Indian travellers.
    // Economy assumes lower hotel/food/transport/shopping.
    // Comfort uses entered values.
    // Premium assumes upgraded stay, food, transport and activities.
    function buildPlan(multiplier, label) {
      const planFare = fareTotal;
      const planHotel = hotelTotal * multiplier.hotel;
      const planFood = foodTotal * multiplier.food;
      const planTransport = numTransportCost * multiplier.transport;
      const planTickets = numTicketCost * multiplier.tickets;
      const planShopping = numShoppingCost * multiplier.shopping;
      const planSubtotal = planFare + planHotel + planFood + planTransport + planTickets + planShopping;
      const planEmergency = planSubtotal * (bufferPercent / 100);
      const planTotal = planSubtotal + planEmergency;

      return {
        label,
        fareTotal: Math.round(planFare),
        hotelTotal: Math.round(planHotel),
        foodTotal: Math.round(planFood),
        transportTotal: Math.round(planTransport),
        ticketTotal: Math.round(planTickets),
        shoppingTotal: Math.round(planShopping),
        emergencyBuffer: Math.round(planEmergency),
        grandTotal: Math.round(planTotal),
        perPerson: Math.round(planTotal / numTravellers)
      };
    }

    const economyPlan = buildPlan({ hotel: 0.75, food: 0.75, transport: 0.75, tickets: 0.85, shopping: 0.60 }, "Economy");
    const comfortPlan = buildPlan({ hotel: 1.00, food: 1.00, transport: 1.00, tickets: 1.00, shopping: 1.00 }, "Comfort");
    const premiumPlan = buildPlan({ hotel: 1.60, food: 1.45, transport: 1.50, tickets: 1.30, shopping: 1.75 }, "Premium");

    let recommendedPlan = "Comfort";
    if (grandTotal <= economyPlan.grandTotal * 1.08) recommendedPlan = "Economy";
    if (grandTotal >= premiumPlan.grandTotal * 0.85) recommendedPlan = "Premium";

    const budgetSummary =
`MyYatraMate Smart Budget Estimate

Destination: ${destination}
Travellers: ${numTravellers}
Travel Mode: ${travelMode || "Not specified"}
Fare per Traveller: ₹${numTravelFare.toLocaleString("en-IN")}
Nights: ${numNights}
Emergency Buffer: ${bufferPercent}%

Cost Breakdown:
Travel Fare Total: ₹${fareTotal.toLocaleString("en-IN")}
...
Emergency Buffer ${bufferPercent}%: ₹${Math.round(emergencyBuffer).toLocaleString("en-IN")}
...
Plan Comparison:
Economy Plan: ₹${economyPlan.grandTotal.toLocaleString("en-IN")} | Per Person: ₹${economyPlan.perPerson.toLocaleString("en-IN")}
Comfort Plan: ₹${comfortPlan.grandTotal.toLocaleString("en-IN")} | Per Person: ₹${comfortPlan.perPerson.toLocaleString("en-IN")}
Premium Plan: ₹${premiumPlan.grandTotal.toLocaleString("en-IN")} | Per Person: ₹${premiumPlan.perPerson.toLocaleString("en-IN")}

Recommended Plan: ${recommendedPlan}

Note:
This is a practical estimate. Actual cost may vary based on travel dates, hotel category, airline/train availability, exchange rate, local transport and personal shopping.`;

    const [result] = await pool.query(
      `INSERT INTO budgets
       (user_id, guest_id, destination, travellers, travel_mode, travel_fare, hotel_cost, nights, food_cost, transport_cost, ticket_cost, shopping_cost, emergency_percent, hotel_total, food_total, emergency_buffer, grand_total, per_person, economy_total, comfort_total, premium_total, recommended_plan, budget_summary)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        owner.userId,
        owner.guestId,
        ...
        grandTotal,
        perPerson,
        economyPlan.grandTotal,
        comfortPlan.grandTotal,
        premiumPlan.grandTotal,
        recommendedPlan,
        budgetSummary
      ]
    );
    ...
    res.json({
      success: true,
      message: "Smart budget calculated successfully.",
      budget: {
        id: result.insertId,
        destination,
        travellers: numTravellers,
        travelMode,
        travelFare: numTravelFare,
        ...
        emergencyPercent: bufferPercent,
        fareTotal,
        hotelTotal,
        foodTotal,
        emergencyBuffer,
    ...
    res.status(500).json({ success: false, message: "Unable to calculate budget: " + error.message });
  }
});
...
app.get("/api/my-budgets", requireUserOrGuest, async (req, res) => {
  try {
    ...
    const [budgets] = await pool.query(
      `SELECT id, destination, travellers, nights, travel_mode AS travelMode, travel_fare AS travelFare,
              emergency_percent AS emergencyPercent, economy_total AS economyTotal, comfort_total AS comfortTotal,
              premium_total AS premiumTotal, recommended_plan AS recommendedPlan,
              grand_total AS grandTotal, per_person AS perPerson,
