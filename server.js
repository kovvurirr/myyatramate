...
Recommended Plan: ${recommendedPlan}`;
    ...
    const [result] = await pool.query(
      `INSERT INTO budgets
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [owner.userId, owner.guestId, destination, numTravellers, travelMode || "", numTravelFare, numHotelCost, numNights, numFoodCost, numTransportCost, numTicketCost, numShoppingCost, bufferPercent, hotelTotal, foodTotal, emergencyBuffer, grandTotal, perPerson, economyPlan.grandTotal, comfortPlan.grandTotal, premiumPlan.grandTotal, recommendedPlan, budgetSummary]
    );
    ...
    res.json({
      ...
      budget: {
        ...
        perPerson,
        recommendedPlan,
        plans: { economy: economyPlan, comfort: comfortPlan, premium: premiumPlan },
        budgetSummary
      }
    });
  } catch (error) {
