...
function buildRuleBasedTripPlan({ fromCity, destination, tripDays, numTravellers, travelType, budgetStyle, foodPref, specialNeeds, travelPace, familyProfile, travelMonth, weatherFocus }) {
  ...
  plan += `Travel Pace: ${travelPace || "Balanced"}\n`;
  plan += `Family Profile: ${familyProfile || "Not specified"}\n`;
  plan += `Travel Month / Dates: ${travelMonth || "Not specified"}\n`;
  plan += `Weather Focus: ${weatherFocus || "General seasonal guidance"}\n`;
  plan += `Special Needs: ${specialNeeds || "None"}\n\n`;
  plan += `Recommended Travel Style:\n`;
  plan += `This trip is planned with focus on practical movement, safety, food preference, budget control, weather-aware suggestions, family comfort and after-landing support.\n\n`;
  ...
  plan += `Weather-aware Suggestions:\n`;
  plan += `- Check the destination weather 24 hours before departure.\n`;
  plan += `- Carry weather-appropriate clothing and keep one backup indoor activity for rain/extreme heat.\n`;
  plan += `- Travel month considered: ${travelMonth || "Not specified"}. Weather focus: ${weatherFocus || "General"}.\n\n`;

  plan += `Family Comfort Suggestions:\n`;
  plan += `- Keep realistic breaks based on family profile: ${familyProfile || "Not specified"}.\n`;
  plan += `- If travelling with seniors or kids, keep attractions closer together and avoid late-night heavy movement.\n\n`;
...
8. Include estimated daily pacing based on the selected travel pace.
9. Include weather-aware suggestions: clothing, backup plan for rain/heat/cold, and best time of day to move around.
10. Include family-profile suggestions: senior citizen comfort, kids breaks, parent-friendly movement, hospital/clinic awareness where relevant.
11. Include a final "MyYatraMate Tips" section.
...
${plan}`;
    ...
    res.json({
      ...
      source: planSource,
      trip: { id: result.insertId, fromCity, destination, days: tripDays, travellers: numTravellers, travelType, budgetStyle, foodPref, specialNeeds, travelPace, familyProfile, travelMonth, weatherFocus, plan }
    });
  } catch (error) {
  ...
  try {
    ...
    const [trips] = await pool.query(
      `SELECT id, from_city AS fromCity, destination, days, travellers, travel_type AS travelType,
              budget_style AS budgetStyle, food_pref AS foodPref, special_needs AS specialNeeds, travel_pace AS travelPace, family_profile AS familyProfile, travel_month AS travelMonth, weather_focus AS weatherFocus,
...
app.put("/api/trip/:id", requireUserOrGuest, async (req, res) => {
  try {
    const owner = getSessionOwner(req);
    const tripId = Number(req.params.id);
    const { destination, days, travellers, travelType, budgetStyle, foodPref, specialNeeds, travelPace, familyProfile, travelMonth, weatherFocus, plan, tripStatus, isFavourite } = req.body;
    ...
    const [result] = await pool.query(
      `UPDATE trip_plans
       SET destination = ?, days = ?, travellers = ?, travel_type = ?, budget_style = ?, food_pref = ?, special_needs = ?,
           travel_pace = ?, family_profile = ?, travel_month = ?, weather_focus = ?, plan = ?,
      ...
      [
        ...
        String(travelPace || "").trim(),
        String(familyProfile || "").trim(),
        String(travelMonth || "").trim(),
        String(weatherFocus || "").trim(),
        String(plan || "").trim(),
        tripStatus || null,
