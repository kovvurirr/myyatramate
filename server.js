...
Per Person: ₹${Math.round(perPerson).toLocaleString("en-IN")}`;
    ...
    const [result] = await pool.query(
      `INSERT INTO budgets
       (user_id, guest_id, destination, travellers, travel_mode, hotel_category, local_transport_mode, shopping_budget, trip_plan_text, ai_budget_json, ai_budget_source, hotel_total, food_total, emergency_buffer, grand_total, per_person, economy_total, comfort_total, premium_total, recommended_plan, budget_summary)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        owner.userId,
        owner.guestId,
...
app.post("/api/workflow-summary", requireUserOrGuest, async (req, res) => {
  try {
    ...
    const budgets = await tryWorkflowQuery(
      `SELECT id, destination, created_at AS createdAt
    ...
    const visaChecklists = await tryWorkflowQuery(
      `SELECT id, destination, passport_country AS passportCountry, created_at AS createdAt
    ...
    const docs = await tryWorkflowQuery(
      `SELECT id, profile_name AS profileName, document_type AS documentType, document_name AS documentName,
